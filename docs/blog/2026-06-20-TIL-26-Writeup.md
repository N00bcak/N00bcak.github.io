...
<!-- 1. Started with RF-DETR and YOLOv8 because those worked well last year.
2. Aggressive augmentation pipeline
  - Marcus and I worked concurrently on RF-DETR; ultimately his came out on top. I pivoted to YOLOv8.
  - Migrating Marcus' augmentation pipeline to YOLOv8 helped it significantly.
3. Increasing YOLO input size to avoid information loss by downsampling: basically several % of free mAP.
4. Weighted box fusion (WBF): ensembling method for bounding boxes. Ensembling RF-DETR and YOLOv8 outputs increased mAP from 0.49 (YOLO-only) to 0.52.
5. Denoiser: DRUNet. 0.52 -> 0.53. However, speed loss was too high to give a new best overall score.
6. TensorRT engine conversion for both object detectors and the denoiser. Inference speed roughly doubled.
  - Qualifiers containers ran on Tesla T4 with compute capability 7.5. Our Vertex AI Workbench instance had a T4 attached, so compilation was straightforward.
  - Finals containers ran on 5070 Ti with compute capability 12.0. Ideally, TRT engines should be compiled using the exact hardware on which they will run, but none of us owned a 5070 Ti. Eventually we realized that TRT had a compilation mode that allowed for compatibility with cards of same compute capability (albeit missing out on some optimizations). Since our team had access to a 5060 and a 5090 (both sm120), we compiled a TRT engine compatible with all sm120 cards and called it a day. -->
...
(BTW here are our params)

- Hyperparameters (where not specified, assume Ultralytics defaults):
    
    | Parameter | Value |
    | --- | --- |
    | Model Variant | RF-DETR 2XLarge |
    | Epochs | 100 |
    | Batch Size | 8 * (grad accum 2) = 16|
    | Image Size | `880 x 880`|
    | Automatic Mixed Precision | True |
    | Learning Rate | 1e-4 |
- Augmentations (in our best-performing version)
...
- Hyperparameters (where not specified, assume Ultralytics defaults):
    ...
    | Parameter | Value |
    | --- | --- |
    | Model Variant | YOLOv8 Large |
    | Epochs | 600 |
    | Batch Size | 42 (Huh.) |
    | Original Image Size | TBD |
    | New Image Size | `1088 x 1920` |
- Augmentations Used (in our best-performing version)
    - Horizontal Flip
    - Affine 
    - Perspective
    - Color Jitter
    ...
    - Grayscale
    - Channel Shuffle
    - Various Blurs (Motion, Gaussian, Median, Defocus, Zoom)
    - Sharpening
    - Various Noises (Gaussian, ISO, Multiplicative)
    - Various weather conditions (Fog, Rain, Shadow, Sun Flare)
    - Resolution Trickery (Image Compression, Downscale, Coarse Dropout)

So this was good. Like, not perfect but decent (`0.49 mAP`). So we went a bit further with these tricks:
- [Weighted Box Fusion](https://arxiv.org/abs/1910.13302): ensembling method for bounding boxes.
- Model ensembling: we ensembled YOLOv8 and RF-DETR outputs to get a final mAP of `0.52`.

Despite all that work, we still had 2 more problems though:
- How would we make our model robust against the noise injection in the finals?
- **Why is our model so fucking slow???**

We tried [DRUNet](https://arxiv.org/pdf/2008.13751) as a denoising preprocessor, but that made our model **EVEN SLOWER** (it timed out!!!)

So instead we decided to focus on speedups instead of trying more crap and making the model slower. Enter **TensorRT**.

I have no idea what happened here, you can ask [Ada](https://scallion3008.github.io/). I suspect she slopped it, but who knows maybe she has 300 IQ.

![](../assets/Images/TIL26/ada.png)

OK. She has 300 IQ.

>  "Finals containers ran on 5070 Ti with compute capability 12.0. Ideally, TRT engines should be compiled using the exact hardware on which they will run, but none of us owned a 5070 Ti. Eventually we realized that TRT had a compilation mode that allowed for compatibility with cards of same compute capability (albeit missing out on some optimizations). Since our team had access to a 5060 and a 5090 (both sm120), we compiled a TRT engine compatible with all sm120 cards and called it a day." ~ Ada

Err.. yeah... whatever she said.
...
Given the absolutely crazy crap that other teams did, I seriously doubt that anyone actually wants to hear what we tried.

I am not kidding. I want you to remember while reading the list below this rule from the competition desc:
> re: The noise must stay within an appropriate SSIM / MSE distance from the original image. Otherwise, **your noise operation is cancelled**.
- Someone lerped the source image with other training samples to create adversarial images. **HOW DID THAT PASS THE FILTER???**
- Another just absolutely mangled the image with like, idk, spiral transforms. That's some [Junji Ito](https://en.wikipedia.org/wiki/Junji_Ito) shit right there.
- Yet another team planted a grid of fake objects. VISIBLE TO THE NAKED EYE. as their noise. **WHAT???**

*cough* anyways, as for how **WE** did noise:
- We recognized (quite obviously) that we did not know **which** model we would be attacking, and that **everyone** (in the finals, anyway) would be rolling up with some kind of finetune.
    - In other words, targeting attacks against **fixed checkpoints** would be a massive waste of time.
- However, we **also** recognized that people were most likely going to use 1 of 2 architectures:
    - CNN (YOLOv8, etc.)
    - Transformer (RF-DETR, GroundingDINO, etc.)

Therefore, our approach to this challenge was: **spam vision-based augmentations until the budget is reached**.
- We first sampled sinusoidal waveforms **of varying frequencies**, to splatter the image with.
    - To minimize budget expenditure, we (and by "we" I mean `GPT-5.5 Pro`) decided to project the waveforms **onto** the direction of increasing luminance in the image. Thus in greyscale, the image **appear** less affected.
- We also added edge blur to screw with CNNs in particular, because that's one of the major ways CNNs work.
    - This was helped by **focusing the random splatter** (mentioned above) **on the edges and interior of objects as opposed to the background**, reducing budget expenditure and increasing the likelihood of affecting the model's perception of the correct objects.
- Lastly, we binary-searched the perturbation magnitude to find the maximum perturbation that would still pass the SSIM/MSE filter.
    - This accounted for JPEG compression artifacts as well so we wouldn't accidentally get *no-oped* for exceeding the budget.

This resulted in a noise operation that really, really messes with human eyes:

![](../assets/Images/TIL26/noise_example.png)

Unfortunately, we didn't really check if it worked on the remote server throughout the competition. In fact, we barely had time getting it to work within the time limit to begin with :P
- Remember how we were all working **AND** settling our other, arguably more important categories? Yeah...

Then Ada 300IQ'd again and spent an entire day speeding it up and moving the procedure to... idk a cuda kernel or something. :P Hooray!
