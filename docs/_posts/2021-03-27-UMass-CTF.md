...
$$\begin{bmatrix}1 & 1\\1 & 0\end{bmatrix}^{m} \begin{bmatrix}F_{n} \\ F_{n-1}\end{bmatrix} = \begin{bmatrix}F_{n+m} \\ F_{n+m-1}\end{bmatrix} \pmod N$$
...
$$\begin{bmatrix}c & -1\\1 & 0\end{bmatrix}^{m} \begin{bmatrix}v_{d,n} \\ v_{d,n-1}\end{bmatrix} = \begin{bmatrix}v_{d,n+m} \\ v_{d,n+m-1}\end{bmatrix} \pmod N$$
...
1. Define two operations, `square` and `multiply`.
    ...
    1b. `multiply` - Assign $$R \leftarrow RP_{original}$$
